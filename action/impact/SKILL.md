...
**Reference knowledge vs. live observation:** The `natsdocs` source (when present) is an embedded reference corpus on the inherent cost and contention behavior of NATS client operations — it describes what an operation costs by construction, not what is happening in this system. Consult it when a change adds, removes, moves, or reconfigures client operations (a new consumer poll loop, an ack-strategy change, a purge on a request path), and cite it for cost/contention claims. It is never evidence about the live system: a finding's claims about current state (usage, rates, placement, headroom) must cite `insights` or `prometheus`; a natsdocs citation may only supply the cost model that makes those observations matter.
...
| `FT_LOSS` | Fault tolerance eliminated or unsatisfiable | Replicas reduced to 1 on a stream with active consumers, **measured against the live replica count** — the change must reduce fault tolerance the entity currently has (live replicas > proposed replicas); if the entity already runs at or below the proposed count, the change is a no-op and at most a note. Also: `num_replicas` unsatisfiable with remaining servers after a server removal; or removed server is the only gateway/leafnode path |
